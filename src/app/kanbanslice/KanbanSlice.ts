...
const kanbanSlice = createSlice({
    ...
    initialState,
    reducers:{
        addCard:(state, action: PayloadAction<{date:string; description:string; status: string; source: string; rating: number; verified: boolean; title:string;}>)=>{
            const cardId = state.cards.length > 0 ? state.cards[state.cards.length - 1].id : 0
            state.cards.push({
                id:cardId + 1,
                ...action.payload
            })
...
export const {addCard, moveCard, deleteCard, editCard} = kanbanSlice.actions
